...
exports.makeInteropBridges = void 0;
const core_1 = require("@idlizer/core");
function withCompute(index, letter, compute) {
    let arg = `${letter}${index}`;
    return compute ? compute(index) : arg;
}
function params(count, letter, compute) {
    if (count == 0)
        return [];
...
function interopConvType(index) {
    return `InteropTypeConverter<P${index}>::InteropType p${index}`;
}
function emitBridge(kind, isVoid, index, _) {
    let maybeV = isVoid ? "V" : "";
...
function makeInteropBridges(kind) {
    let _ = new core_1.IndentedPrinter();
    let args = kind.split(":");
    if (args.length != 2)
        throw new Error(`Format is "kind:count"`);
...
exports.makeInteropBridges = makeInteropBridges;
//# sourceMappingURL=InteropBridges.js.map