...
exports.arkoalaLayout = exports.KotlinLayout = exports.CJLayout = exports.JavaLayout = exports.ArkTSComponentsLayout = exports.ArkTsLayout = exports.TsLayout = exports.HandwrittenModule = exports.SyntheticModule = void 0;
...
exports.SyntheticModule = "./SyntheticDeclarations";
function HandwrittenModule(language, isSdk = false) {
    // does this switch needed here?
    switch (language) {
        case core_1.Language.TS: return "./handwritten";
        case core_1.Language.ARKTS: return isSdk ? './handwritten' : "#handwritten";
        case core_1.Language.KOTLIN: return "./handwritten";
        default: throw new Error("Not implemented");
    }
}
exports.HandwrittenModule = HandwrittenModule;
function toFileName(name) {
    return name.split(/[_-]/gi).map(it => idl.capitalize(it)).join('');
}
function customPathSuggestion(pkg) {
    const suggestions = (0, libohos_1.peerGeneratorConfiguration)().currentModulePackagesPaths;
    if (!suggestions)
        return undefined;
...
function getModuleImport(node, role, lang) {
    var _a, _b;
    if (role == core_1.LayoutNodeRole.GLOBAL)
        return undefined;
    if (idl.isInCurrentModule(node))
        return undefined;
    if (idl.isInExternalModule(node)) {
        if (role == core_1.LayoutNodeRole.SERIALIZER)
            return undefined;
    }
    const conf = (0, libohos_1.peerGeneratorConfiguration)();
...
class TsLayout extends CommonLayoutBase {
    ...
    resolve(target) {
        ...
        if (target.node.name === libohos_1.NativeModule.Generated.name)
            return getGeneratedFilePath(`peers/${libohos_1.NativeModule.Generated.name}`);
        if (idl.isHandwritten(target.node) || (0, libohos_1.peerGeneratorConfiguration)().isHandWritten(target.node.name)) {
            return HandwrittenModule(this.library.language);
        }
        // if (idl.isSyntheticEntry(target.node)) {
        //     return SyntheticModule
        // }
        const moduleImport = getModuleImport(target.node, target.role, core_1.Language.TS);
        if (moduleImport)
            return moduleImport;
        if (idl.isInterface(target.node) && !(0, libohos_1.isComponentDeclaration)(this.library, target.node)) {
...
class ArkTsLayout extends CommonLayoutBase {
    ...
    handwrittenPackage() {
        return HandwrittenModule(core_1.Language.ARKTS, this.isSdk);
    }
    resolve(target) {
        var _a, _b, _c, _d;
        if (target.node.name === libohos_1.NativeModule.Generated.name)
        ...
        const moduleImport = getModuleImport(target.node, target.role, core_1.Language.ARKTS);
        if (moduleImport)
            return moduleImport;
        let customPath;
...
exports.ArkTsLayout = ArkTsLayout;
class ArkTSComponentsLayout extends ArkTsLayout {
    ...
    resolve(target) {
        if (target.node.name === libohos_1.NativeModule.Generated.name)
            return getGeneratedFilePath(`arkts/${libohos_1.NativeModule.Generated.name}`);
        return super.resolve(target);
    }
}
exports.ArkTSComponentsLayout = ArkTSComponentsLayout;
class JavaLayout extends CommonLayoutBase {
    ...
    resolve({ node, role }) {
        switch (role) {
            case core_1.LayoutNodeRole.SERIALIZER:
            case core_1.LayoutNodeRole.INTERFACE: {
                if (idl.isEntry(node)) {
                ...
                if (idl.isInterface(node)) {
                    if ((0, libohos_1.isComponentDeclaration)(this.library, node)) {
                    ...
                    if ((0, core_1.isMaterialized)(node, this.library)) {
            ...
            case core_1.LayoutNodeRole.PEER: {
                if (idl.isInterface(node)) {
                    if ((0, libohos_1.isComponentDeclaration)(this.library, node)) {
            ...
            case core_1.LayoutNodeRole.GLOBAL: {
                return 'GlobalScope';
            }
            case core_1.LayoutNodeRole.COMPONENT: {
                return 'Ark' + node.name;
            }
        }
    }
}
exports.JavaLayout = JavaLayout;
...
class KotlinLayout extends CommonLayoutBase {
    ...
    resolve(target) {
        ...
        if (idl.isSyntheticEntry(target.node)) {
            return exports.SyntheticModule;
        }
...
exports.KotlinLayout = KotlinLayout;
////////////////////////////////////////////////////////
...
exports.arkoalaLayout = arkoalaLayout;
