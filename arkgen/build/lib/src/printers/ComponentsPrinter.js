...
exports.printComponentsDeclarations = exports.printComponents = exports.generateAttributeModifierSignature = exports.generateArkComponentName = void 0;
...
function generateArkComponentName(component) {
    return `Ark${component}Component`;
}
exports.generateArkComponentName = generateArkComponentName;
function expandComponentWithSupers(library, decl) {
    const result = [];
    while (decl) {
        const superResolved = (0, core_1.getSuper)(decl, library);
        result.push(decl);
        decl = superResolved;
    }
    return result;
}
function generateAttributeModifierSignature(library, component) {
    const modifiers = expandComponentWithSupers(library, component.attributeDeclaration).map(it => idl.createReferenceType((0, knownReferences_1.getReferenceTo)('AttributeModifier'), [idl.createReferenceType((0, PeersPrinter_1.componentToAttributesInterface)(it.name))]));
    return new core_1.NamedMethodSignature(idl.IDLThisType, [idl.createUnionType([...modifiers, idl.IDLUndefinedType])], 
    // [idl.createOptionalType(modifiers.length > 1 ? idl.createUnionType(modifiers) : modifiers[0])],
    ['value']);
}
exports.generateAttributeModifierSignature = generateAttributeModifierSignature;
...
class TSComponentFileVisitor {
    ...
    overloadsPrinter(printer) {
        return new libohos_1.OverloadsPrinter(this.library, printer, this.library.language, true, this.library.useMemoM3);
    }
    visit() {
        const result = [];
        (0, libohos_1.collectPeersForFile)(this.library, this.file).forEach(peer => {
    ...
    printImports(peer, component) {
        ...
        (0, libohos_1.collectDeclItself)(this.library, idl.createReferenceType((0, knownReferences_1.getReferenceTo)('AttributeModifier')), imports);
        (0, libohos_1.collectDeclItself)(this.library, idl.createReferenceType((0, knownReferences_1.getReferenceTo)('AttributeUpdater')), imports);
        if (!this.options.isDeclared) {
            ...
            imports.addFeature((0, libohos_1.componentToPeerClass)(peer.componentName), this.library.layout.resolve({ node: component.attributeDeclaration, role: core_1.LayoutNodeRole.PEER }));
        }
        if (peer.originalParentFilename) {
            ...
            while (parentDecl) {
                ...
                imports.addFeatures([
                    (0, libohos_1.componentToStyleClass)(parentComponent.attributeDeclaration.name),
                    (0, PeersPrinter_1.componentToAttributesInterface)(parentComponent.attributeDeclaration.name),
                ], `./${parentGeneratedPath}`);
        ...
        (0, libohos_1.collectDeclDependencies)(this.library, component.attributeDeclaration, imports);
        if (component.interfaceDeclaration)
            (0, libohos_1.collectDeclDependencies)(this.library, component.interfaceDeclaration, imports);
        return imports;
    }
    ...
    printComponent(peer) {
        ...
        printer.writeClass(componentClassName, (writer) => {
            writer.writeMethodImplementation(new core_1.Method('getPeer', new core_1.MethodSignature((0, core_1.createReferenceType)(peerClassName), []), [core_1.MethodModifier.PROTECTED], []), writer => writer.writeStatement(writer.makeReturn(writer.makeCast(writer.makeFieldAccess("this", "peer"), (0, core_1.createReferenceType)(peerClassName), { optional: true }))));
            for (const grouped of (0, libohos_1.groupOverloads)(peer.methods, this.library.language))
            ...
            writer.writeMethodImplementation(new core_1.Method('attributeModifier', generateAttributeModifierSignature(this.library, component), [core_1.MethodModifier.PUBLIC]), writer => {
                writer.writeStatement(writer.makeReturn(writer.makeThis()));
            });
            const attributesFinishSignature = new core_1.MethodSignature(core_1.IDLVoidType, []);
            const applyAttributesFinish = 'applyAttributesFinish';
            writer.writeMethodImplementation(new core_1.Method(applyAttributesFinish, attributesFinishSignature, [core_1.MethodModifier.PROTECTED]), (writer) => {
        ...
        }, parentComponentClassName, [(0, PeersPrinter_1.componentToAttributesInterface)(peer.originalClassName)]);
        return [{
                collector: imports,
                content: printer,
                over: {
    ...
    printComponentFunctions(peer) {
        ...
        const componentInterfaceName = (0, PeersPrinter_1.componentToAttributesInterface)(peer.originalClassName);
        ...
        const collapsedCallables = (0, libohos_1.allowsOverloads)(this.library.language)
            ? callableMethods
            : callableMethods.length > 0
                ? [(0, libohos_1.collapseSameNamedMethods)(callableMethods)]
                : [];
        collapsedCallables.forEach(callableMethod => {
            ...
            const peerClassName = (0, libohos_1.componentToPeerClass)(peer.componentName);
            if (!((_a = (0, libohos_1.collectComponents)(this.library).find(it => it.name === component.name)) === null || _a === void 0 ? void 0 : _a.interfaceDeclaration))
                return [{
            ...
            if (paramsList)
                paramsList += ",";
            printer.writeLines((0, libohos_1.readLangTemplate)(`component_builder_${declaredPostrix}${stagePostfix}`, this.library.language)
...
class JavaComponentFileVisitor {
    ...
    visit() {
        (0, libohos_1.collectPeersForFile)(this.library, this.file).forEach(peer => this.printComponent(peer));
        return [];
    }
    getComponentResults() {
        return [];
    ...
    printComponent(peer) {
        const componentClassName = generateArkComponentName(peer.componentName);
        const componentType = (0, core_1.createReferenceType)(componentClassName);
        const parentComponentClassName = peer.parentComponentName ? generateArkComponentName(peer.parentComponentName) : libohos_1.COMPONENT_BASE;
        const peerClassName = (0, libohos_1.componentToPeerClass)(peer.componentName);
        const result = this.library.createLanguageWriter(core_1.Language.JAVA);
        result.print(`package ${libohos_1.ARKOALA_PACKAGE};\n`);
        const imports = (0, libohos_1.collectJavaImports)(peer.methods.flatMap(method => method.method.signature.args));
        ...
        result.writeClass(componentClassName, (writer) => {
            peer.methods.forEach(peerMethod => {
                ...
                const signature = new core_1.NamedMethodSignature(componentType, originalSignature.args, originalSignature.argsNames, originalSignature.defaults);
                const method = new core_1.Method(peerMethod.method.name, signature, [core_1.MethodModifier.PUBLIC]);
            ...
            const applyAttributesSignature = new core_1.MethodSignature(core_1.IDLVoidType, []);
            const applyAttributes = 'applyAttributes';
            writer.writeMethodImplementation(new core_1.Method(applyAttributes, applyAttributesSignature, [core_1.MethodModifier.PROTECTED]), (writer) => {
        ...
        this.results.push(new ComponentPrintResult(new libohos_1.TargetFile(componentClassName + core_1.Language.JAVA.extension, libohos_1.ARKOALA_PACKAGE_PATH), result));
    }
}
class CJComponentFileVisitor {
    ...
    filterOverloadsForCallback(overloads, paramName, writer) {
        ...
        const hasInlineFunction = overloads.some(alt => {
            ...
            return CJCallbackTypeManager_1.CJCallbackTypeManager.isFunctionType(typeName);
        });
        if (hasInlineFunction) {
            ...
            const filteredOverloads = overloads.filter(alt => {
                ...
                // 过滤掉内联函数类型：(args) -> Ret
                if (CJCallbackTypeManager_1.CJCallbackTypeManager.isFunctionType(typeName)) {
    ...
    formatParameter(name, typeName, defaultValue, isOptional) {
        ...
        if (CJCallbackTypeManager_1.CJCallbackTypeManager.isCallbackType(typeName)) {
            return CJCallbackTypeManager_1.CJCallbackTypeManager.formatCallbackParameter(name, typeName, isOptional);
        }
    ...
    processParameter(name, typeName, defaultValue, isOptional) {
        const isCallback = CJCallbackTypeManager_1.CJCallbackTypeManager.isCallbackType(typeName);
    ...
    processAndFormatParameter(paramType, index, method, componentName, writer, overrideType, baseConversion) {
        ...
        try {
            ...
            try {
                ...
                if (!CJCallbackTypeManager_1.CJCallbackTypeManager.isCallbackType(tName) && CJCallbackTypeManager_1.CJCallbackTypeManager.isFunctionType(originalTypeName)) {
    ...
    formatBuildFunctionParams(componentName) {
        ...
        return params.map(param => {
            const wrappedType = CJCallbackTypeManager_1.CJCallbackTypeManager.wrapCallbackType(param.type, param.isOptional);
            // 构建函数入参：根据类型智能设置默认值
    ...
    scoreMethodForCallbackPreference(methodGroup, writer, componentName) {
        ...
        method.signature.args.forEach((paramType, index) => {
            try {
                ...
                // 检测内联函数类型（归一化后检测）
                else if (CJCallbackTypeManager_1.CJCallbackTypeManager.isFunctionType(typeName)) {
                    score -= 5;
                    inlineFunctionCount++;
                }
            }
    ...
    printComponent(peer) {
        ...
        printer.writeClass(componentClassName, (writer) => {
            writer.writeMethodImplementation(new core_1.Method('getPeer', new core_1.MethodSignature((0, core_1.createReferenceType)(peerClassName), []), [core_1.MethodModifier.PROTECTED], []), writer => {
                writer.print('if (let Some(peer) <- this.peer) {');
                writer.pushIndent();
                writer.writeStatement(writer.makeReturn(writer.makeCast(writer.makeString("peer"), (0, core_1.createReferenceType)(peerClassName), { optional: true })));
                writer.popIndent();
            ...
            // todo stub until we can process AttributeModifier
            if ((0, core_1.isCommonMethod)(peer.originalClassName) || peer.originalClassName == "ContainerSpanAttribute")
    ...
    printComponentFunction(peer) {
        ...
        const stagePostfix = this.library.useMemoM3 ? "m3" : "m1";
        let paramsList = mappedCallableParams === null || mappedCallableParams === void 0 ? void 0 : mappedCallableParams.join(", ");
        let templateContent = (0, libohos_1.readLangTemplate)(`component_builder_${declaredPostrix}${stagePostfix}`, this.library.language)
            .replaceAll("%COMPONENT_NAME%", component.name)
            .replaceAll("%COMPONENT_ATTRIBUTE_NAME%", componentInterfaceName)
...
function printComponents(peerLibrary) {
    return new ComponentsVisitor(peerLibrary, { isDeclared: false }).printComponents();
}
exports.printComponents = printComponents;
function printComponentsDeclarations(peerLibrary) {
    // TODO: support other output languages
    if (![core_1.Language.TS, core_1.Language.ARKTS, core_1.Language.JAVA].includes(peerLibrary.language))
        return [];
    return new ComponentsVisitor(peerLibrary, { isDeclared: true }).printComponents();
}
exports.printComponentsDeclarations = printComponentsDeclarations;
