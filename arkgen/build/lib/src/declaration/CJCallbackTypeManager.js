...
exports.CJCallbackTypeManager = void 0;
/**
 * 回调类型命名化管理器
 *
 * 职责：
 ...
 * 4. 支持基于仓颉语言类型系统的回调处理
 */
class CJCallbackTypeManager {
    constructor() {
        // 回调别名注册表：规范化函数签名 -> 命名类型
        this.callbackAliasBySignature = new Map();
        // 组件级别的回调类型定义
        this.componentCallbackDefinitions = new Map();
    }
    /**
     * 清理当前组件的回调类型缓存
     */
    clearComponentCallbacks(componentName) {
     ...
     */
    processCallbackType(paramName, typeName, methodName, componentName) {
     ...
     * 获取或创建回调类型别名
     */
    getOrCreateCallbackAlias(functionSignature, paramName, methodName, componentName) {
        // 规范化函数签名作为缓存键
        ...
        this.componentCallbackDefinitions.get(componentName).add(`public type ${aliasName} = ${normalizedSignature}`);
        return aliasName;
    }
    /**
     * 规范化函数签名
     */
    normalizeFunctionSignature(signature) {
        return signature
            .replace(/\s+/g, ' ') // 统一空白字符
     ...
     * 3. 确保唯一性：添加数字后缀避免冲突
     */
    generateCallbackAliasName(paramName, methodName, componentName) {
        let baseName = '';
     ...
     * 获取指定组件的回调类型定义
     */
    getCallbackDefinitions(componentName) {
     ...
     * 检查是否为函数类型
     */
    static isFunctionType(typeName) {
     ...
     * 检测是否为回调类型（命名回调或内联函数）
     */
    static isCallbackType(typeName) {
        const s = typeName.trim();
        // 1) 命名回调类型：OnXxxCallback
        if (/\b\w+Callback\b$/.test(s))
            return true;
        // 2) Option<OnXxxCallback>
        if (/^Option<\s*\w+Callback\s*>$/.test(s))
            return true;
        // 3) 内联函数类型
        if (CJCallbackTypeManager.isFunctionType(s))
            return true;
        // 4) 历史遗留 CallbackCallback 也视为回调，便于替换
        if (/\bCallbackCallback\b$/.test(s))
            return true;
        return false;
    }
    /**
     ...
     */
    static extractBaseCallbackType(typeName) {
     ...
     * 根据可选性包装回调类型
     */
    static wrapCallbackType(baseType, isOptional) {
        return isOptional ? `Option<${baseType}>` : baseType;
    }
    /**
     * 格式化回调参数（统一入口）
     */
    static formatCallbackParameter(name, typeName, isOptional) {
     ...
     * 确保回调类型被 Option<> 包装
     */
    static ensureOptionCallbackType(typeName) {
     ...
     */
    prescanMethod(method, componentName, typeMapper, typeRewriter) {
        if (!method)
            return;
        const perParamConversions = method.signature.args.map((paramType, idx) => {
     ...
     * 批量预扫描方法数组
     */
    prescanMethods(methods, componentName, typeMapper, typeRewriter) {
...
exports.CJCallbackTypeManager = CJCallbackTypeManager;
// Option包装正则，编译一次复用
CJCallbackTypeManager.OPTION_WRAPPER_REGEX = /^Option<\s*(.+)\s*>$/;
//# sourceMappingURL=CJCallbackTypeManager.js.map