...
import { CanvasRenderingContext2D, CanvasRenderingContext2D_serializer, CanvasRenderingContext2DInternal } from "@arkoala/arkui/generated"
...
function checkReadAndMutateBuffer() {
    ...
    const serializer = SerializerBase.hold()
    const nativeBuffer = new NativeBuffer(uint8array.length)
    InteropNativeModule._CopyArray(nativeBuffer.data, nativeBuffer.length, uint8array)
    serializer.writeBuffer(nativeBuffer)
    TestNativeModule._TestReadAndMutateManagedBuffer(serializer.toArray(), serializer.length())

    let isSame = true
    for (let i = 0; i < bufferSize; ++i) {
        isSame = isSame && (i + 1) * 2 === nativeBuffer.readByte(i)
    }
    serializer.release()
