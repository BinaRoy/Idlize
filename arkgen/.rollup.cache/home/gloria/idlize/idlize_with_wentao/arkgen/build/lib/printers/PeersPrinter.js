...
import { throwException, Language, InheritanceRole, determineParentRole, isHeir, generatorConfiguration, LayoutNodeRole, ArgumentModifier, capitalize, createConstructPeerMethod, Method, MethodModifier, NamedMethodSignature, createReferenceType, IDLI32Type, IDLPointerType, IDLStringType, IDLVoidType, } from '@idlizer/core';
import { ImportsCollector, ARKOALA_PACKAGE, collectJavaImports, printJavaImports, collectDeclDependencies, collectDeclItself, findComponentByName, findComponentByType, NativeModule, collectPeersForFile, writePeerMethod } from "@idlizer/libohos";
import { HandwrittenModule } from '../ArkoalaLayout';
import { CJTypeMapper } from '../declaration/CJTypeMapper';
export function componentToPeerClass(component) {
    return `Ark${component}Peer`;
}
export function componentToStyleClass(component) {
    if (component.endsWith("Attribute"))
        component = component.substring(0, component.length - 9);
    return `Ark${component}Style`;
}
export function componentToAttributesInterface(component) {
    return `${component}`;
}
// For TS and ArkTS
class PeerFileVisitor {
    ...
    generatePeerParentName(peer) {
        ...
        const parent = (_b = peer.parentComponentName) !== null && _b !== void 0 ? _b : throwException(`Expected component to have parent`);
    ...
    printPeerConstructor(peer, printer) {
        ...
        const signature = new NamedMethodSignature(IDLVoidType, [IDLPointerType, IDLI32Type, IDLStringType, IDLI32Type], ['peerPtr', 'id', 'name', 'flags'], [undefined, undefined, '""', '0']);
        printer.writeConstructorImplementation(componentToPeerClass(peer.componentName), signature, (writer) => { }, { delegationArgs: ['peerPtr', 'id', 'name', 'flags'].map(it => printer.makeString(it)), delegationName: peer.parentComponentName }, [MethodModifier.PROTECTED]);
    }
    printCreateMethod(peer, writer) {
        const peerClass = componentToPeerClass(peer.componentName);
        const signature = new NamedMethodSignature(createReferenceType(peerClass), [createReferenceType('ComponentBase'), IDLI32Type], ['component', 'flags'], [undefined, '0'], [[ArgumentModifier.OPTIONAL], undefined]);
        writer.makeStaticBlock(() => {
            writer.writeMethodImplementation(new Method('create', signature, [MethodModifier.STATIC, MethodModifier.PUBLIC]), (writer) => {
                ...
                writer.writeStatement(writer.makeAssign(_peerPtr, undefined, writer.makeNativeCall(NativeModule.Generated, `_${peer.componentName}_${createConstructPeerMethod(peer).sig.name}`, [writer.makeString(peerId), writer.makeString(signature.argName(1))]), true));
                const _peer = '_peer';
...
class CJPeerFileVisitor extends PeerFileVisitor {
    ...
    * 创建重载方法对象
    ...
    createOverloadedMethod(method, overloadIndex, overloadAlt) {
        ...
        // 创建新的方法签名
        const newSignature = new NamedMethodSignature(signature.returnType, newArgs, signature.argsNames, signature.defaults, signature.argsModifiers);
        // 创建新的方法对象
        const newMethod = new Method(method.method.name, newSignature, method.method.modifiers, method.method.generics);
        // 创建新的 PeerMethod
...
export function createPeersPrinter(dumpSerialized) {
    return (library) => new PeersVisitor(library, dumpSerialized).printPeers();
}
export function generateStyleParentClass(peer) {
    if (!isHeir(peer.originalClassName))
        return undefined;
