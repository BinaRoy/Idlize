...
import { IfStatement, isHeir, Language, LayoutNodeRole, Method, MethodModifier, MethodSignature } from "@idlizer/core";
import { componentToPeerClass, findComponentByDeclaration, findComponentByName, groupOverloads, ImportsCollector, peerGeneratorConfiguration } from "@idlizer/libohos";
...
class ModifiersFileVisitor {
    ...
    printModifiers(peer) {
        ...
        const attributeFilter = (name) => {
            var _a, _b;
            const hookRecord = (_b = peerGeneratorConfiguration().hooks.get((_a = peer.originalClassName) !== null && _a !== void 0 ? _a : '')) === null || _b === void 0 ? void 0 : _b.get(name);
        ...
        printer.writeClass(this.generateAttributeSetName(componentAttribute.name), (writer) => {
            ...
            writer.writeMethodImplementation(new Method(`setInstanceId`, new MethodSignature(idl.IDLVoidType, [idl.IDLNumberType], [], [], [], ['instanceId'])), writer => {
            ...
            writer.writeMethodImplementation(new Method('applyModifierPatch', new MethodSignature(idl.IDLVoidType, [idl.createReferenceType(componentToPeerClass(peer.componentName))], [], [], [], ['peer'])), writer => {
                if (parentSet)
                ...
                attributeTypes.forEach((attribute) => {
                    ...
                    switchPrinter.popIndent();
                    switchPrinter.print(`}`);
                    statements.push(new IfStatement(writer.makeString(expr), writer.makeBlock(switchPrinter.getOutput().map(s => writer.makeStatement(writer.makeString(s)))), undefined, undefined, undefined));
                });
...
export function printModifiers(peerLibrary) {
