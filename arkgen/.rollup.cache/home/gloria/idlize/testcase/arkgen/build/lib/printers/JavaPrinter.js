...
import { Method, MethodModifier, MethodSignature, NamedMethodSignature } from "@idlizer/core";
...
export function makeJavaArkComponents(library) {
    const ark = 'Ark';
        ...
        library.files.forEach(file => {
            collectPeersForFile(library, file).forEach(peer => {
                ...
                writer.writeMethodImplementation(method, writer => {
                    ...
                    writer.writeStatement(writer.makeAssign(create, idl.createReferenceType(`Supplier<${arkPeer}>`), writer.makeLambda(new MethodSignature(arkPeerType, []), [
                        writer.makeReturn(writer.makeString(`${arkPeer}.create(${receiver}, 0)`))
                    ]), true));
                    writer.writeStatement(writer.makeAssign(update, idl.createReferenceType(`Consumer<${arkPeer}>`), writer.makeLambda(new NamedMethodSignature(idl.IDLVoidType, [arkPeerType], ['peer']), (callableMethod ?
