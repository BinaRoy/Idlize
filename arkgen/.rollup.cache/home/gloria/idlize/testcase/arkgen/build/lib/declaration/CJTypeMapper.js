...
import { convertNumberProperty } from './CJNumberConversion';
...
export class CJTypeMapper {
    ...
    applyUnionConvergenceRules(flags, paramName, isOptional) {
        ...
        if (flags.hasString && flags.hasNumber) {
            ...
            try {
                const numMapping = convertNumberProperty({ propertyName: paramName, isOptional });
                return {
                    overloads: [
        ...
        if (flags.hasNumber) {
            try {
                const numMapping = convertNumberProperty({ propertyName: paramName, isOptional: true });
    ...
    convertNumberType(baseType, paramName) {
        try {
            const result = convertNumberProperty({
    ...
    convertUnionType(memberTypes, paramName, isOptional) {
        try {
            ...
            if (hasNumber && hasArray) {
                ...
                return {
                    overloads: [
                        { cjType: scalar, defaultValue: DEFAULT_VALUES[scalar] },
            ...
            if (hasNumber && hasResource) {
                try {
                    ...
                    const resBranch = hasResourceStr ? 'ResourceStr' : 'Resource';
                    return { overloads: [{ cjType: 'Float64', defaultValue: DEFAULT_VALUES.FLOAT64 }, { cjType: resBranch }], isFromUnionOverload: true };
                }
            }
            ...
            if (hasString && hasNumber) {
                ...
                try {
                    const numMapping = convertNumberProperty({ propertyName: paramName, isOptional: isOptional });
                    const overloads = [
                        { cjType: numMapping.cjType, defaultValue: numMapping.defaultValue },
                        { cjType: 'String', defaultValue: DEFAULT_VALUES.RESOURCE_STR }
                    ];
